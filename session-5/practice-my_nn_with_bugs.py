...
def load_mnist_from_csv(train_csv_path, test_csv_path, val_split=0.1):
    ...
    X_train_full = train_data.iloc[:, 1:].to_numpy(np.float32)

    # PLEASE CHECK
    y_test = test_data.iloc[:, 0].to_numpy(np.int64)
    X_test = test_data.iloc[:, 1:].to_numpy(np.float32)
...
class Dense(Layer):
    ...
    def __init__(self, input_units, output_units, learning_rate=0.1):
        ...
        # PLEASE CHECK
        self.weights = np.random.randn(output_units, input_units) * np.sqrt(
            2.0 / input_units
        )
        # PLEASE CHECK
        self.biases = np.zeros(input_units)

    def forward(self, input):
        self.input = input
        # ASSERT: check input dimension
        assert input.shape[1] == self.weights.shape[1], "Input dimension mismatch!"
        # PLEASE CHECK
...
def train_mnist_network(X_train, y_train, X_val, y_val, num_epochs=5):
    network = [
        Dense(784, 64),
        # PLEASE CHECK
        ReLU(64, 64),
