...
5. Write the bias-corrected first moment formula $\hat m$.
6. Write the bias-corrected second moment formula $\hat v$.
7. Write the Adam parameter update rule for $W$.
...
2. How does the second moment help when different directions have different gradient scales?
3. Draw an optimization landscape with one steep direction and one shallow direction.
4. Explain why Adam can be easier to tune than plain SGD in many practical neural-network problems.
5. Give one reason Adam is not always guaranteed to give the best final generalization.
