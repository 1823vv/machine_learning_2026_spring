...
You train a neural network on MNIST. The input dimension is $28\times 28$, and the output has 10 classes. After many epochs, training accuracy is very high, but validation accuracy is much lower.
...
7. Draw a workflow diagram showing:

   ```text
   choose hyperparameters -> train -> monitor validation -> select best model -> final test once
   ```

8. Explain why the final test set should be used only after model selection is finished.

## Question: Parameter Counting Reminder

Consider a fully connected MNIST network:
