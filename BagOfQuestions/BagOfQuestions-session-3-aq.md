## Question: DeepSeek Mini Mixture-of-Experts with Simple Counting

A simplified DeepSeek-inspired model uses a tiny mixture-of-experts layer. The fancy idea is that the model stores several small expert networks, but for one token it may use only some of them. For this question, focus only on counting weights and biases.
