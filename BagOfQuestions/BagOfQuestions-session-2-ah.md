...
1. **Stage 1 (Raw features, no hidden layer).** Use only $x_1, x_2$ and no hidden layers. Write the model score
   ...
   z = w_1x_1 + w_2x_2 + b,
   $$
   then explain why this boundary class cannot separate two intertwined spirals. Explicitly distinguish **optimization difficulty** from **representation limitation**.
...
3. **Stage 3 (Add depth).** Keep the same input features and add hidden layers with tanh activations, for example one hidden layer (6--8 neurons), then two or three layers (about 8--12 neurons each). Explain how compositions
   $$
