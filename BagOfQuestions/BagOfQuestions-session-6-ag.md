...
2. Write a regularized objective using $\mathcal{L}_{data}(W)$ plus an L2 penalty.
3. Explain the role of $\lambda$.
4. What happens when $\lambda=0$?
5. What can happen when $\lambda$ is extremely large?
6. Explain why regularization can improve validation performance even if it increases training loss.
...
2. Which penalty, L1 or L2, is especially associated with sparse weights?
3. Which penalty, L1 or L2, smoothly shrinks many weights toward zero?
4. Draw a simple comparison of unregularized weights and regularized weights.
5. Explain why regularization strength should be selected using validation data.
